...
def select(disabled: bool = False) -> LLMTier:
    ...
    Order: explicitly disabled -> live Claude if an API key is present (in the
    environment, or in the repository `.env`) -> recorded
    ...
    if disabled:
        ...
        return NullTier()
    # A key in `.env` and a key in the environment must select the same tier. Without
    # this, whether the live tier ran depended on how the process happened to be
    # launched -- and the metrics block would print `recorded` next to the numbers with
