...
def test_defect_category_count_matches(batch):
    ...
    m = _claim(r"\*\*(\w+) categories\*\*, each ground-truth labelled")
    claimed = words.get(m.group(1))
    assert claimed is not None, f"unrecognised number word {m.group(1)!r}"
    actual = len({lab for link in batch.truth for lab in link.defect_labels})
    assert claimed == actual, (
        f"README claims {claimed} defect categories; the generator emits {actual}"
    )
